...
class Coche(private val _marca: String, private val _modelo: String, private var _velocidadActual: Int) {
  ...
  def acelerar(): Unit = {
     _velocidadActual = _velocidadActual +10
      ...
      // Método acelerar2
  def acelerar(speed: Int): Unit = {
     if((_velocidadActual+speed)>180){
        println("No se puede acelerar tanto")
     } else {
          _velocidadActual += speed
    }

    // Método frenar
  def frenar(): Unit = {
    if ((_velocidadActual -40) >= 0) _velocidadActual = _velocidadActual -40
    else println("Error: La velocidad no puede ser negativa")
    }
